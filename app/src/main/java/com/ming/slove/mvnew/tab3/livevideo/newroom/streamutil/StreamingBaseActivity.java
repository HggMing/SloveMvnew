...
//        Log.e(TAG, "selected size :" + size.width + "x" + size.height);
        return null;//// FIXME: 2017/2/6 修改为空，不使用上面的自定义方案
    }
...
//                mShutterButtonPressed = false;
    ...
    private void saveToSDCard(String filename, Bitmap bmp) throws IOException {
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            ...
            final String info = "截图保存到:" + Environment.getExternalStorageDirectory().getAbsolutePath() + "/" + filename;
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
