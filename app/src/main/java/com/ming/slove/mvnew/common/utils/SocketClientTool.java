...
public class SocketClientTool {
    ...
    public SocketClient getLocalSocketClient() {
        if (this.mSocketClient == null) {
            ...
            __i__setupAddress(this.mSocketClient);
            __i__setupEncoding(this.mSocketClient);

            __i__setupConstantHeartBeat(this.mSocketClient);

//            __i__setupVariableHeartBeat(this.mSocketClient);

//            __i__setupReadToTrailerForSender(this.mSocketClient);
//            __i__setupReadToTrailerForReceiver(this.mSocketClient);

            __i__setupReadByLengthForSender(this.mSocketClient);
            __i__setupReadByLengthForReceiver(this.mSocketClient);

//            __i__setupReadManuallyForSender(this.mSocketClient);
//            __i__setupReadManuallyForReceiver(this.mSocketClient);

            this.mSocketClient.registerSocketClientDelegate(new SocketClientDelegate() {
                @Override
            ...
            this.mSocketClient.registerSocketClientSendingDelegate(new SocketClientSendingDelegate() {
            ...
            this.mSocketClient.registerSocketClientReceiveDelegate(new SocketClientReceivingDelegate() {
                @Override
    ...
    private void __i__setupAddress(SocketClient socketClient) {
        socketClient.getAddress().setRemoteIP("118.178.232.77"); // 远程端IP地址
        socketClient.getAddress().setRemotePort("7272"); // 远程端端口号
    ...
    private void __i__setupReadByLengthForSender(SocketClient socketClient) {
         ...
         * 此转换器用于第二步
         *
         * 使用{@link SocketPacketHelper.ReadStrategy.AutoReadByLength}必须设置此项
         * 用于分隔多条消息
         */
         ...
         * 若无需包尾可删除此行
         * 注意：
         * 使用{@link SocketPacketHelper.ReadStrategy.AutoReadByLength}时不依赖包尾读取数据
         */
        socketClient.getSocketPacketHelper().setSendTrailerData(new byte[]{0x13, 0x10});
